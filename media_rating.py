import os
import pandas as pd
from google.cloud import bigquery

os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = "C:/Users/RITHIN REDDY/Downloads/halogen-goods-457111-g4-3fcb6d8aecd4.json"

df = pd.read_csv(r"C:/Users/RITHIN REDDY/Downloads/movie_genre_classification_final.csv")

client = bigquery.Client()
job_config = bigquery.LoadJobConfig(write_disposition="WRITE_TRUNCATE")


df_director = df[['Director']].drop_duplicates().reset_index(drop=True)
df_director['director_id'] = df_director.index + 1
df_director = df_director[['director_id', 'Director']].rename(columns={'Director': 'director_name'})
table_id = "halogen-goods-457111-g4.peerislands.Director"
job = client.load_table_from_dataframe(df_director, table_id, job_config = job_config)
job.result()


df_actor = df[['Lead_Actor']].drop_duplicates().reset_index(drop=True)
df_actor['actor_id'] = df_actor.index + 1
df_actor = df_actor[['actor_id', 'Lead_Actor']].rename(columns={'Lead_Actor': 'actor_name'})
table_id = "halogen-goods-457111-g4.peerislands.Actor"
job = client.load_table_from_dataframe(df_actor, table_id, job_config = job_config)
job.result()


df_company = df[['Production_Company']].drop_duplicates().reset_index(drop=True)
df_company['company_id'] = df_company.index + 1
df_company = df_company[['company_id', 'Production_Company']].rename(columns={'Production_Company': 'company_name'})
table_id = "halogen-goods-457111-g4.peerislands.Company"
job = client.load_table_from_dataframe(df_company, table_id, job_config = job_config)
job.result()


df_genre = df[['Genre']].drop_duplicates().reset_index(drop=True)
df_genre['genre_id'] = df_genre.index + 1
df_genre = df_genre[['genre_id', 'Genre']].rename(columns={'Genre': 'genre_name'})
table_id = "halogen-goods-457111-g4.peerislands.Genre"
job = client.load_table_from_dataframe(df_genre, table_id, job_config = job_config)
job.result()


df_language = df[['Language']].drop_duplicates().reset_index(drop=True)
df_language['language_id'] = df_language.index + 1
df_language = df_language[['language_id', 'Language']].rename(columns={'Language': 'language_name'})
table_id = "halogen-goods-457111-g4.peerislands.Language"
job = client.load_table_from_dataframe(df_language, table_id, job_config = job_config)
job.result()


df_country = df[['Country']].drop_duplicates().reset_index(drop=True)
df_country['country_id'] = df_country.index + 1
df_country = df_country[['country_id', 'Country']].rename(columns={'Country': 'country_name'})
table_id = "halogen-goods-457111-g4.peerislands.Country"
job = client.load_table_from_dataframe(df_country, table_id, job_config = job_config)
job.result()


df_fact = df.copy()
df_fact = df_fact.merge(df_director, left_on='Director', right_on='director_name', how='left')
df_fact = df_fact.merge(df_actor, left_on='Lead_Actor', right_on='actor_name', how='left')
df_fact = df_fact.merge(df_company, left_on='Production_Company', right_on='company_name', how='left')
df_fact = df_fact.merge(df_genre, left_on='Genre', right_on='genre_name', how='left')
df_fact = df_fact.merge(df_language, left_on='Language', right_on='language_name', how='left')
df_fact = df_fact.merge(df_country, left_on='Country', right_on='country_name', how='left')
# df_fact = df_fact.merge(df_rating, left_on='Content_Rating', right_on='content_rating', how='left')
df_fact_final = df_fact[[
    'Title',
    'Year',
    'Description',
    'Duration',
    'Rating',
    'Votes',
    'Budget_USD',
    'BoxOffice_USD',
    'Num_Awards',
    'Critic_Reviews',
    'director_id',
    'actor_id',
    'company_id',
    'genre_id',
    'language_id',
    'country_id',
    
]].rename(columns={
    'Duration': 'duration_minutes',
    'Votes': 'votes',
    'Budget_USD': 'budget_usd',
    'BoxOffice_USD': 'box_office_usd',
    'Num_Awards': 'num_awards',
    'Critic_Reviews': 'critic_reviews'
})

table_id = "halogen-goods-457111-g4.peerislands.fact_movies"
job = client.load_table_from_dataframe(df_fact_final, table_id, job_config = job_config)
job.result()
print("Job Done")